#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Vendored from the okf-open-knowledge-format skill (author: ft.ia.br).
# Licensed under Apache-2.0.
# Source repo: https://github.com/fabricioctelles/skills
#
# This is the authoritative OKF v0.1 conformance oracle for this project's
# bundles. Do not diverge from upstream; re-vendor to update.
# ---------------------------------------------------------------------------
...
while IFS= read -r -d '' file; do
  ...
  # NOTE (vendoring patch): upstream used `head -n -1` to drop the closing `---`,
  # which is GNU-only and aborts on macOS/BSD `head`. `sed '$d'` is portable and
  # equivalent (delete the last line). Behavior is identical on GNU and BSD.
